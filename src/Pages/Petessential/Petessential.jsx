...
const Petessential = () => {
    ...
    const handlesubmit = async (e) => {
        e.preventDefault();
        if (!name || !image || !redirect_to) return;
        ...
        if (editid) {
            try {
                const response = await putwithformdata(`pet_essential/${editid}`, formData);
                toast.success("pet essential update successfully!");
                setData((prevData) =>
                    prevData.map((item) =>
            ...
            try {
                const response = await Postwithformdata('pet_essential', formData);
                toast.success("pet essential add successfully!");
                if (response && response.error == 0) {
    ...
    const fetchpetessential = async () => {
        try {
            const response = await getWithoutHeader('pet_essential');
    ...
    const handledit = (id) => {
        ...
        if (found) {
            ...
            setredirect_to(found.redirect_to);
        } else {
    ...
    useEffect(() => {
        startTransition(fetchpetessential);
    }, []);
    const handleDelete = async (id) => {
        if (confirm('Are you sure you want to delete?')) {
            try {

                const response = await deleteapi(`pet_essential/${id}`);
                if (response && response.error === 0) {
                ...
                <div className="container">
                    <form onSubmit={handlesubmit}>
                        <div className="grid grid-cols-4 mt-3 gap-3 items-center">
                            ...
                            <div className="col-span-1">
                                <label className="block text-[#001B48] font-bold mb-2">Redirect to</label>
                                <input
                                    type="text"
                                    value={redirect_to}
                                    onChange={(e) => setredirect_to(e.target.value)}
                                    className="w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#001B48]"
